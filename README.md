...
**Automatic, without a key:** NHS Jobs official XML feeds (including an explicit UKHSA government source), public Greenhouse, Lever, Ashby and SmartRecruiters employer endpoints, and bounded personal-use retrieval of jobs.ac.uk listings and their structured vacancy metadata. Employer connectors are configurable in Source Health. Feeds and custom authorised employer JSON-LD pages can be added there.
...
Paste the full advert, including essential/desirable headings, when an import cannot read the page. LinkedIn and Indeed automated retrieval is deliberately disabled. A low request rate does not grant permission for automation under [Indeed’s terms](https://www.indeed.com/legal). Importing the same URL enriches the existing record. Every record retains its source links. When an original employer advert merges with an aggregator result, a recognised direct employer URL is preferred.

**Indeed assisted import:** Search → **Indeed · browse and bring back an advert** lets you choose search terms, location and recency, then opens Indeed in your browser. Copy a vacancy URL and the full advert into **Paste an Indeed advert**. Fieldwork prepares a local draft; explicit `Job title:`, `Employer:`, `Location:` and salary labels are recognised, while unknown fields stay blank. Review the draft before adding it. No Indeed account, cookie, browser automation or API key is used. Importing text does not mark a vacancy open or establish suitability. Different `jk`/`vjk` and tracking links for the same Indeed job merge with the saved vacancy, preserving applications; different job keys remain distinct. Manual imports never count as automatic source checks.

**Employer catalogue and careers URLs:** Source Health includes an optional catalogue adapted from the supplied [AI Job Search project](https://github.com/MadsLorentzen/ai-job-search), with its [MIT attribution](THIRD_PARTY_NOTICES.md). Synthesia, Improbable and Wayve are included automatically; the remaining catalogue employers can be connected individually. Add a source → **Identify connector** recognises hosted Greenhouse, Lever (including EU), Ashby and SmartRecruiters board URLs. Review the employer name and choose **Add source**. Connecting the same board twice does not duplicate its schedule. The catalogue is discovery coverage, not a recommendation that its jobs suit you.

Ashby uses its documented public posting API, excludes unlisted posts and retains published salary, location and work-pattern information. `publishedAt` is the last-published date, not necessarily the original posting date. Generic remote posts do not establish UK eligibility. No upstream AI assistant, Bun runtime or scraper is needed.
