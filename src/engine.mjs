...
export function extractRequirements(j) {
  ...
  for(let line of String(j.description||'').split(/\n|(?<=[.!?])\s+/)){
    ...
    if(/^(?:benefits|about us|what we offer|our (?:team|mission)|responsibilities|equal opportunit|as an equal opportunity|interviewing with|how to apply|application process|#LI-)/i.test(line)){section='unspecified';continue;}
  ...
  for(const key of ['essential','desirable'])if(Array.isArray(explicit[key])&&explicit[key].length)out[key]=[...new Set([...out[key],...explicit[key]])];
  return out;
}
function yearsIn(s){
...
export function assess(job, config={}, at=new Date()) {
  ...
  if(/account executive|sales (?:representative|manager|director)|business development|recruitment consultant|customer success manager/i.test(title))barriers.push('Commercial or sales role rather than a scientific / analytical role');
  ...
  if(senior)barriers.push('Title indicates senior or doctoral-level expertise');
  ...
  for(const line of essential){
    if(/\bmaster(?:'|’)?s?\b|\bMSc\b|\bMRes\b/i.test(line)&&!/bachelor|\bbsc\b|or equivalent experience/i.test(line)&&!/master|msc|mres/.test(knownQualifications))gaps.push('A Master’s degree is required; postgraduate training alone does not confirm this qualification');
  ...
  for(const skill of ['SQL','SAS','C++','Java','ArcGIS','Power BI','Tableau','AWS','GCP','SAP']){
    ...
    if(pattern.test(hard)&&!cfg.skills.some(x=>x.toLowerCase()===skill.toLowerCase()))missingTechnical.push(skill);
  }
  if(/\bR\b/.test(hard)&&!cfg.skills.includes('r'))missingTechnical.push('R');
  ...
  const methods=matched.filter(x=>['python','pandas','numpy','statistics','data analysis','machine learning','visualisation','scientific computing','databases'].includes(x));
  ...
  gates.requirements=barriers.length===0&&unspecifiedYears<=cfg.max_experience&&(reviewed||(essential.length>=2&&uncovered.length===0&&missingTechnical.length===0))&&desc.length>=250;
  gates.open=open;
  ...
  return {category,score,family,early_career:early,matched,positive,issues:[...new Set(issues)].slice(0,15),barriers:[...new Set(barriers)],gates,requirements:req,unreviewed_requirements:uncovered,preference,engine_version:1};
}
